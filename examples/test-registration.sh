...
echo "Creating token for kuiper-tart-agent..."
...
echo ""
echo "2. In terminal 2, bootstrap and start the kuiper-tart-agent:"
echo "   cargo run --bin kuiper-tart-agent -- \\"
...
echo ""
echo "=== For kuiper-proxmox-agent ==="
echo ""
echo "Create a new token and use --token flag:"
echo "   cargo run --bin kuiper-forge -- token create --labels linux,x64"
echo "   cargo run --bin kuiper-proxmox-agent -- --config examples/kuiper-proxmox-agent-config.toml --token REG_TOKEN"
echo ""
echo "=== Notes ==="
echo ""
